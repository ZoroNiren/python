...
BOT_TOKEN = os.getenv("BOT_TOKEN", "8537740701:AAFh44HaA19cUiDBGM1ozGydh7yZCNo9jAQ")
...
TEST_MODE = _env_flag("BOT_TEST_MODE", DEFAULT_TEST_MODE)
...
QUARTER_TEST_DELAY_SECONDS = 20 * 60 if TEST_MODE else 40
...
WARNING_TEXT = (
    "\n\n⚠️ Пожалуйста, не редактируйте отправленные сообщения.\n"
    "Если вы допустили ошибку — вы сможете исправить её через меню. Подробнее в инструкции."
)
commands_texts = {
    "command1": "Инструкция", 
...
async def handle_command(message: types.Message):
    logging.info(f"[handle_command] user_id={message.from_user.id}, text={message.text!r}")
    ...
    # ✅ НОВОЕ: блокируем команды если пользователь занят
    # command1 (инструкция) разрешаем всегда
    if cmd != "command1" and is_user_busy(user_id):
        await message.answer(
            "⚠️ Вы находитесь в процессе заполнения данных. Пожалуйста, завершите полностью текущий этап перед использованием этой команды. "
            "После того как Вы заполните все необходимые поля (бот перестанет отправлять Вам сообщения), Вы сможете вносить изменения через меню и соответствующие команды." + WARNING_TEXT
        )
        return
...
async def handle_command_alias(message: types.Message):
    ...
    if len(parts) != 2 or not parts[1].isdigit():
        await message.reply("Используйте формат /command5 или /command 5."+ WARNING_TEXT)
        return

    command_number = int(parts[1])
    if command_number < 1 or command_number > 14:
        await message.reply("Команда должна быть в диапазоне от 1 до 14."+ WARNING_TEXT)
        return
...
def _restore_state_payload_for_user(user_id: int, payload: dict) -> None:
    ...
    mapping_pairs = [
        ...
        ("income_reminder_state", income_reminder_state),
        ("report_files_state", report_files_state),
        ("task_check_state", task_check_state),
        ("new_tasks_state", new_tasks_state),
        ("quarter_goal_state", quarter_goal_state),
    ...
    if user_id in task_check_state:
        _restore_tasks_as_tuples(task_check_state[user_id])
    if user_id in new_tasks_state:
        _restore_tasks_as_tuples(new_tasks_state[user_id])


def load_chat_state_for_user(user_id: int) -> bool:
    try:
        conn = get_connection()
        cursor = conn.cursor()
        cursor.execute("SELECT state_json FROM chat_state WHERE user_id = ?", (user_id,))
        row = cursor.fetchone()
        conn.close()
        ...
        _restore_state_payload_for_user(user_id, payload)
        return has_any_active_state(user_id, check_db=False)
    except Exception as e:
        logging.exception(f"⛔️ Ошибка восстановления состояния чата для user_id={user_id}: {e}")
        return False
...
def load_chat_state_all() -> None:
    ...
    try:
        conn = get_connection()
        cursor = conn.cursor()
        cursor.execute("SELECT user_id, state_json FROM chat_state")
        ...
        for row in rows:
            try:
                ...
                _restore_state_payload_for_user(db_user_id, payload)
                if has_any_active_state(db_user_id, check_db=False):
                    restored_count += 1
            except Exception as inner_e:
                logging.exception(f"⛔️ Ошибка восстановления состояния пользователя {row[0]}: {inner_e}")

        logging.info(f"ℹ️ Восстановлено интерактивных состояний после перезапуска: {restored_count}")
    except Exception as e:
        logging.exception(f"⛔️ Ошибка массового восстановления состояний: {e}")


def has_any_active_state(user_id: int, check_db: bool = True) -> bool:
    active_maps = [
        user_state,
        expense_flow,
        income_flow,
        reminder_state,
        income_reminder_state,
        report_files_state,
        task_check_state,
        new_tasks_state,
        quarter_goal_state,
    ...
    try:
        ...
        has_payload = any(payload.get(k) for k in state_keys)
        if has_payload:
            _restore_state_payload_for_user(user_id, payload)
            return True
    except Exception as e:
        logging.exception(f"⛔️ Ошибка проверки активного состояния для user_id={user_id}: {e}")
...
finance_categories_kb = ReplyKeyboardMarkup(
    keyboard=[
...
async def handle_edit_command(message: Message, user_id: int, command: str) -> bool:
    ...
    if command == "command2":
        ...
        await message.answer(
            f"Старое название проекта: {project_name}\n\nВведите новое название проекта."+ WARNING_TEXT
        )
        return True
    ...
    if command == "command13":
        ...
        await message.answer(
            f"Старая выписка банка: {file_name}\n\nОтправьте новый файл или фото."+ WARNING_TEXT
        )
        return True
    ...
    await message.answer(
        f"Текущие данные ({prompt}):\n{old_values}\n\nНапишите номер записи, которую хотите изменить."+ WARNING_TEXT
    )
    return True
...
async def request_documents_if_needed_or_finish(message: Message, user_id: int, project_id: int, closed_period_id: int | None) -> None:
    if project_requires_documents(project_id):
        ...
        await message.answer(
            ...
            "Подойдут фото (JPG, PNG) или PDF."+ WARNING_TEXT,
...
async def start_new_tasks_creation(
    ...
    await message.answer(
        ...
        "❗️Когда закончите - напишите «Готово»."+ WARNING_TEXT,
        reply_markup=ReplyKeyboardRemove(),
    )
...
async def prompt_expense_step(target: Message | None, user_id: int) -> None:
    ...
    if index < len(categories):
        ...
        text = (
            "✅ Все категории расходов заполнены.\n"
            "Если хотите добавить новую категорию расходов — нажмите «Добавить категорию».\n"
            "Если всё заполнено — нажмите «Готово»."
        )
        kb = finance_categories_kb
...
async def prompt_income_step(target: Message | None, user_id: int) -> None:
    ...
    if index < len(categories):
        ...
        text = (
            "✅ Все категории доходов заполнены.\n"
            "Если хотите добавить новую категорию доходов — нажмите «Добавить категорию».\n"
            "Если всё заполнено — нажмите «Готово»."
        )
        kb = finance_categories_kb
    if target is not None:
...
async def finalize_expenses(message: Message, user_id: int, state: dict) -> None:
    ...
    logging.info(f"[reminder_task] reminder_state after set: {user_id in reminder_state}")
...
async def process_financial_result_and_check_tasks(message: Message, user_id: int) -> None:
    ...
    if old_tasks:
        ...
        await message.answer(
            ...
            "Выберите, пожалуйста, статус задачи:"+ WARNING_TEXT,
...
async def trigger_quarter_goal_check(user_id: int, project_id: int) -> None:
    ...
    await bot.send_message(
        ...
        f"Цель: {goal_text}\n\n"
        "Укажите, достигли ли вы её:"+ WARNING_TEXT,
...
async def _ensure_onboarded(message: Message, user_id: int) -> bool:
    if is_user_onboarded(user_id):
        return True
    await message.answer(
        "Сначала выполните /start, чтобы получить инструкцию и начать заполнение.",
        reply_markup=ReplyKeyboardRemove(),
    )
    return False
...
async def onboarding_accept_handler(message: Message) -> None:
    logging.info(f"[onboarding_accept_handler] user_id={message.from_user.id}")
    ...
    # ✅ НОВОЕ: если уже онбордился и занят — не сбрасывать состояние
    if is_user_onboarded(user_id) and is_user_busy(user_id):
        await message.answer(
            "⚠️ Вы уже ознакомились с инструкцией и сейчас в процессе заполнения данных."+ WARNING_TEXT
        )
        return
    ...
    await message.answer("Введите, пожалуйста, название проекта"+ WARNING_TEXT, reply_markup=ReplyKeyboardRemove())
...
async def start_handler(message: Message) -> None:
    ...
    # ✅ НОВОЕ: блокируем /start если пользователь в процессе заполнения
...
async def handle_files(message: Message) -> None:
    ...
    if user_id in report_files_state:
        ...
        if QUARTER_TEST_MODE:
            ...
            asyncio.create_task(_delayed_check(user_id, state["project_id"]))
...
async def save_message_legacy_disabled(message: Message) -> None:
    ...
    logging.info(f"[save_message] user_id={user_id}, text={text!r}, reminder_state={user_id in reminder_state}")
    ...
    if step == "awaiting_project_name" and text and not text.startswith("/"):
        ...
        touch_user_activity(user_id)
        save_chat_state_for_user(user_id)
        await message.answer("💵 Введите, пожалуйста, сумму денег на расчетных счетах"+ WARNING_TEXT)
        return
    ...
    if user_id in edit_state:
        ...
        if step == "awaiting_selection":
            if not text.isdigit():
                await message.answer("Пожалуйста, отправьте номер записи из списка."+ WARNING_TEXT)
                return

            selected_index = int(text) - 1
            items = state.get("items", [])
            if selected_index < 0 or selected_index >= len(items):
                await message.answer("Такого номера нет. Выберите номер записи из списка."+ WARNING_TEXT)
                return
        ...
        if step == "awaiting_new_value":
            ...
            if command == "command2":
                ...
                if new_value is None or new_value < 0:
                    conn.close()
                    await message.answer("❌ Введите корректное неотрицательное число."+ WARNING_TEXT)
                    return
                ...
                if not new_status:
                    conn.close()
                    await message.answer("Пожалуйста, выберите статус из кнопок."+ WARNING_TEXT, reply_markup=task_status_kb)
                    return
                selected_item = state["selected_item"]
    ...
    if user_id in quarter_goal_state:
        ...
        if state.get("step") == "awaiting_quarter_status":
            ...
            if text == "❌ Не достигнуто":
                ...
                save_chat_state_for_user(user_id)
                await message.answer(
                    "Укажите, пожалуйста, причину, по которой цель не была достигнута:"+ WARNING_TEXT,
                    reply_markup=ReplyKeyboardRemove(),
                )
                return

            await message.answer(
                "Пожалуйста, выберите один из вариантов:"+ WARNING_TEXT,
                reply_markup=quarter_status_kb,
            )
            return
            ...
            if text in ["✅ Сделано", "🔄 В процессе", "❌ Не сделано"]:
                ...
                if current_index < len(tasks):
                    ...
                    if status == "not_done":
                        ...
                        save_chat_state_for_user(user_id)
                        await message.answer(
                            "Укажите причину, по которой задача не выполнена:"+ WARNING_TEXT,
                            reply_markup=ReplyKeyboardRemove(),
                        )
                        return
    ...
    if user_id in new_tasks_state:
        ...
        await message.answer("Задача добавлена ✅\n\n"
                             "Введите следующую или «Готово»"+ WARNING_TEXT)
        return
    ...
    if user_id in income_reminder_state:
        ...
        if normalize_text(text) in READY_WORDS:
            await message.answer("❌ На этом этапе нужно вводить суммы по категориям, а не «Готово»."+ WARNING_TEXT, reply_markup=ReplyKeyboardRemove())
            return

        amount = parse_float_from_user(text)
        if amount is None:
            await message.answer("❌ Пожалуйста, введите число без смайликов, букв и других символов."+ WARNING_TEXT, reply_markup=finance_categories_kb)
            return

        if amount < 0:
            await message.answer("❌ Число не может быть отрицательным"+ WARNING_TEXT, reply_markup=finance_categories_kb)
            return
        ...
        await message.answer("Нажмите «Добавить категорию» или «Готово»."+ WARNING_TEXT, reply_markup=finance_categories_kb)
        return
    ...
    await notify_admin(
        "🆕 Новый проект зарегистрирован!\n"
        f"Название: {project_name}\n"
        "Пользователь указал основные данные проекта."+ WARNING_TEXT
    )
    await notify_admin(
        "🎯 Заполнена цель проекта на квартал!\n"
        f"Название: {project_name}\n"
...
async def save_message(message: Message) -> None:
    logging.info(f"[save_message ENTRY] user_id={message.from_user.id}, text={message.text!r}")
    if is_duplicate_message(message):
        return
    ...
    if user_id in edit_state:
        ...
        if step == "awaiting_new_value":
            ...
            if command == "command2":
                ...
                if not latest_period:
                    conn.close()
                    await message.answer("Не удалось найти период для обновления."+ WARNING_TEXT)
                    return
                _, _, _, deposit_balance = latest_period
                cursor.execute(
                    """
    ...
    if user_id in income_reminder_state:
        amount = parse_float_from_user(text)
        if amount is None:
            await message.answer("❌ Пожалуйста, введите число без смайликов, букв и других символов."+ WARNING_TEXT)
            return
        
        if amount < 0:
            await message.answer("❌ Число не может быть отрицательным"+ WARNING_TEXT)
            return
        ...
        if index < len(categories):
            ...
            if state["current_index"] < len(categories):
                ...
                await message.answer(f"💳 Введите сумму доходов по категории: {next_cat}"+ WARNING_TEXT)
            else:
                await process_financial_result_and_check_tasks(message, user_id)
        return
    ...
    if user_id in reminder_state:
        ...
        if index < len(categories):
            ...
            if state["current_index"] < len(categories):
                next_cat = categories[state["current_index"]]
                await message.answer(f"💸 Введите сумму расходов по категории: {next_cat}"+ WARNING_TEXT)
            else:
                conn = get_connection()
                cursor = conn.cursor()
                ...
                if income_categories:
                    ...
                    save_chat_state_for_user(user_id)
                    await message.answer(f"💳 Введите сумму доходов по категории: {income_categories[0]}"+ WARNING_TEXT)
                else:
                    income_reminder_state[user_id] = {
                        "step": "awaiting_income_amount",
                        "categories": [],
    ...
    if user_id not in user_state:
        if not is_user_onboarded(user_id) and not has_any_active_state(user_id):
            await message.answer("Используйте /start для создания проекта" + WARNING_TEXT)
        return
    ...
    if step == "awaiting_accounts":
        ...
        save_chat_state_for_user(user_id)
        await message.answer("💵 Введите, пожалуйста, сумму денег на депозитах"+ WARNING_TEXT)
        return
    ...
    if step == "awaiting_income_categories":
        if normalize_text(text) in READY_WORDS:
            ...
            await message.answer(
                ...
                "Каждую категорию вводите отдельным сообщением.\n" 
                "❗️Как введете все имеющиеся категории - напишите «Готово»"+ WARNING_TEXT,
                parse_mode="HTML"
            )
            return
...
async def reminder_task() -> None:
    while True:
        ...
        for period_id, user_id, project_name, project_id in rows:
            logging.info(f"[reminder_task] user_id={user_id}, is_busy={is_user_busy(user_id)}, "
                        f"user_state={user_id in user_state}, "
                        f"reminder_state={user_id in reminder_state}, "
                        f"report_files_state={user_id in report_files_state}")
            ...
            logging.info(f"[reminder_task] sent={sent}, user_id={user_id}")
            if not sent:
                cursor.execute("UPDATE periods SET reminder_sent = 1 WHERE id = ?", (period_id,))
                conn.commit()
                continue

            logging.info(f"[reminder_task] categories запрос для project_id={project_id}")
            cursor.execute(
                "SELECT name FROM categories WHERE project_id = ? AND type='expense'",
                (project_id,),
            )
            categories = [row[0] for row in cursor.fetchall()]
            logging.info(f"[reminder_task] categories={categories}")
            ...
            logging.info(f"[reminder_task] reminder_state SET: {reminder_state.get(user_id)}")
...
async def documents_request_task() -> None:
    """Отдельный таймер документов отключён.
    Документы запрашиваются только по сценарию после новых задач,
    чтобы запрос не дублировался сразу после успешной загрузки файлов.
    """
    while True:
        await asyncio.sleep(2 if TEST_MODE else 60)
        continue
